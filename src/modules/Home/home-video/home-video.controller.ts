import { Body, Controller, Delete, Get, Param, Patch, Post, Query, UploadedFiles, UseGuards, UseInterceptors } from "@nestjs/common";
import { Constants } from "src/utils/constants";
import { FileFieldsInterceptor } from "@nestjs/platform-express";
import { AdminAuthGuard } from "src/core/guards/admin-auth.guard";
import { UploadedMulterFileI } from "src/modules/SpacesModule/SpacesService";
import { StaticRoutesProps } from "src/core/decorators/static-routes-props.decorator";
import { HomeVideoService } from "./home-video.service";
import { CreateHomeVideoDto } from "./dto/create-home-video.dto";
import { UpdateHomeVideoDto } from "./dto/update-home-video.dto";

@Controller({ path: "home-video", version: Constants.API_VERSION_1 })

export class HomeVideoController {
    constructor(private readonly service: HomeVideoService) {
    }

    @Post()
    @StaticRoutesProps({ routeName: 'home-video' })
    @UseGuards(AdminAuthGuard)
    @UseInterceptors(FileFieldsInterceptor(
        [
            { name: "image", maxCount: 1 },
            { name: "video", maxCount: 1 }
        ]
    ))
    create(@Body() dto: CreateHomeVideoDto,
        @UploadedFiles() files: {
            image?: UploadedMulterFileI,
            video?: UploadedMulterFileI
        }) {
        return this.service.create(dto, files);
    }

    @Get()
    @StaticRoutesProps({ routeName: 'home-video' })
    @UseGuards(AdminAuthGuard)
    findAll(@Query('page') pageParam?: string,
    @Query('limit') limitParam?: string) {
        return this.service.findAll(pageParam, limitParam);
    }

    @Get(":id")
    @StaticRoutesProps({ routeName: 'home-video' })
    @UseGuards(AdminAuthGuard)
    findOne(@Param("id") id: string) {
        return this.service.findOne(id);
    }

    @Patch("/:id")
    @StaticRoutesProps({ routeName: 'home-video' })
    @UseGuards(AdminAuthGuard)
    @UseInterceptors(FileFieldsInterceptor(
        [
            { name: "image", maxCount: 1 },
            { name: "video", maxCount: 1 }
        ]
    ))
    update(@Param("id") id: string, @Body() dto: UpdateHomeVideoDto,
        @UploadedFiles() files: {
            image?: UploadedMulterFileI,
            video?: UploadedMulterFileI
        }) {
        return this.service.update(id, dto, files);
    }

    @Delete(":id")
    @StaticRoutesProps({ routeName: 'home-video' })
    @UseGuards(AdminAuthGuard)
    async remove(@Param("id") id: string) {
        return await this.service.delete(id);
    }
}
